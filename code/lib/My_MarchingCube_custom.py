...
class My_MarchingCube():
    ...
    def __init__(self, Size, Res, Iso):
        """
        Constructor
    ...
    def runGPU(self, VolGPU, GPUManager):
        """
        ...
        # Create the programs
        GPUManager.programs['MarchingCubesIndexing'] = cl.Program(GPUManager.context, KernelsOpenCL.Kernel_MarchingCubeIndexing).build()
        GPUManager.programs['MarchingCubes'] = cl.Program(GPUManager.context, KernelsOpenCL.Kernel_MarchingCube).build()
        
        # Convert 3D volume into 1D array voume for GPU computation
        Size_Volume = cl.Buffer(GPUManager.context, mf.READ_ONLY | mf.COPY_HOST_PTR, \
        ...
        OffsetGPU = cl.Buffer(GPUManager.context, mf.READ_WRITE, tmp.nbytes)
        IndexGPU = cl.Buffer(GPUManager.context, mf.READ_WRITE, tmp.nbytes)
        FaceCounterGPU = cl.Buffer(GPUManager.context, mf.READ_WRITE | mf.COPY_HOST_PTR, hostbuf = self.nb_faces)
        
        ParamGPU = cl.Buffer(GPUManager.context, mf.READ_ONLY | mf.COPY_HOST_PTR, \
                               hostbuf = self.res)
        self.nb_faces[0] = 0
        # Create Buffer for the number of faces
        cl.enqueue_write_buffer(GPUManager.queue, FaceCounterGPU, self.nb_faces).wait()
        # Run indexing edges algorithm
        GPUManager.programs['MarchingCubesIndexing'].MarchingCubesIndexing(GPUManager.queue, (self.Size[0]-1, self.Size[1]-1), None, \
                                VolGPU, OffsetGPU, IndexGPU, Size_Volume, np.int32(self.iso), FaceCounterGPU).wait()
        
        # read buffer to know number of faces
        cl.enqueue_read_buffer(GPUManager.queue, FaceCounterGPU, self.nb_faces).wait()
        ...
        FacesGPU = cl.Buffer(GPUManager.context, mf.READ_WRITE, self.Faces.nbytes)
        VerticesGPU = cl.Buffer(GPUManager.context, mf.READ_WRITE, self.Vertices.nbytes)

        # Compute faces and vertices.
        GPUManager.programs['MarchingCubes'].MarchingCubes(GPUManager.queue, (self.Size[0]-1, self.Size[1]-1), None, \
                                VolGPU, OffsetGPU, IndexGPU, VerticesGPU, FacesGPU, ParamGPU, Size_Volume).wait()

        # Read buffer array into numpy array
        cl.enqueue_read_buffer(GPUManager.queue, VerticesGPU, self.Vertices).wait()
        cl.enqueue_read_buffer(GPUManager.queue, FacesGPU, self.Faces).wait()
